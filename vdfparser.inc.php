				...
				switch($c) {
					...
					case CURLY_BRACE_BEGIN:
						...
						if(!$reading) {
							...
							$arrayLevel = count($ptr);
							$ptr[$arrayLevel] = array(KEYTITLE => $key);
							$ptr = &$ptr[$arrayLevel];
							...
							if($path == '')
								...
								$path .= PATH_DEPTH_SEPARATOR . $arrayLevel;
					...
					case CURLY_BRACE_END:
						...
						if(!$reading) {
							...
							// //空白kv检测
							// if (count($ptr)==1){
							// 	print "delete empty array".$ptr[KEYTITLE];
							// 	unset($ptr);
							// }
...
/*
	* VDF (Valve Data Format) file writer
	* author: CrazyKid
	* version: 0.1
*/

function VDFWrite($parsedArray, $file) {
	$tabCount = 0; //tab缩进个数

	$myfile = fopen($file, "w");
	foreach ($parsedArray[0] as $key => $value) {
		parseKv($parsedArray[0][$key], $key, $value, $tabCount, $myfile);
	}

	$tabCount--;
	$text = buildTab($tabCount).CURLY_BRACE_END.PHP_EOL; //结尾括号
	fwrite($myfile, $text);
...
function parseKv($val, $key, $value, &$tabCount, $file){
	//是kv title
	if (!is_array($val)){
		if ($key===KEYTITLE){ //是key-title
			$text = buildTab($tabCount).QUOTE.$value.QUOTE.PHP_EOL;
			fwrite($file, $text);

			$text = buildTab($tabCount).CURLY_BRACE_BEGIN.PHP_EOL; //开始括号
			fwrite($file, $text);
			$tabCount++;
		}
		else //是单个key-value
		{
			$text = buildTab($tabCount).QUOTE.$key.QUOTE."	".QUOTE.$value.QUOTE.PHP_EOL;
			fwrite($file, $text);
		}
	}
	//是key-value
		...
		foreach ($val as $key2 => $value2) {
			parseKv($val[$key2], $key2, $value2, $tabCount, $file);
		}
		if (array_key_exists(KEYTITLE, $val)){
			$tabCount--;
			$text = buildTab($tabCount).CURLY_BRACE_END.PHP_EOL; //结尾括号
			fwrite($file, $text);
		}
